# working properly with alarm
import cv2
from ultralytics import YOLO
from pathlib import Path
import threading
from pydub import AudioSegment
from pydub.playback import play  # Using pydub for sound playback
...
# Alarm sound path
alarm_sound_path = 'alarm.wav'  # Path to the alarm sound file (use full path if necessary)

# Function to play the alarm sound in a separate thread
def play_alarm():
    try:
        print("Playing alarm sound...")  # Debug statement
        alarm_sound = AudioSegment.from_wav(alarm_sound_path)  # Load the sound file
        play(alarm_sound)  # Play the sound
...
def detect_fire_from_rtsp(rtsp_url):
    ...
    while cap.isOpened():
        ...
        if len(results) > 0 and len(results[0].boxes) > 0:
            for result in results:
                for box in result.boxes:
                    ...
                    print(f"Detected class: {class_name}, Confidence: {confidence:.2f}")  # Debug print
        ...
        if fire_detected and not alarm_triggered:
            ...
            print("Fire detected! Triggering alarm.")  # Debug statement
